...
const CONTENT_CATEGORIES = {
  ...
  FEAR: { label: "Scared", icon: Clock, color: "bg-yellow-100 text-yellow-800" },
  SURPRISE: { label: "Surprise", icon: Users, color: "bg-green-100 text-green-800" },
  DISGUST: { label: "Disgust", icon: Filter, color: "bg-purple-100 text-purple-800" },
}
 ...
 */
function getQSVCClassification(video: any): { categories: string[]; score: number } {
  ...
  if (title.includes("learn") || title.includes("alphabet") || title.includes("numbers") || title.includes("story")) {
    predictedEmotion = "JOY"
  } else if (title.includes("bedtime") || title.includes("relax")) {
    predictedEmotion = "SADNESS"
  } else if (title.includes("muddy") || title.includes("fun") || title.includes("party")) {
          ...
          <div className="flex flex-wrap items-center gap-4 mb-6 p-4 bg-muted/50 rounded-lg">
            ...
            <Select value={contentFilter} onValueChange={setContentFilter}>
              ...
              <SelectContent>
                ...
                {Object.keys(CONTENT_CATEGORIES).map((key) => (
                  <SelectItem key={key} value={key}>{CONTENT_CATEGORIES[key as keyof typeof CONTENT_CATEGORIES].label}</SelectItem>
                ))}
              </SelectContent>
            </Select>
            ...
            <div className="text-sm text-muted-foreground">Showing {filteredVideos.length} videos</div>
          </div>
        )}

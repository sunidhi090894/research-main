...
export async function GET() {
  try {
    ...
    const videos = records.map((record: any, index: number) => ({
      ...
      category: inferCategory(record.title, record.description),
    }))
...
function inferCategory(title: string, description: string): string {
  ...
  if (text.includes('learning') || text.includes('alphabet') || text.includes('numbers') || text.includes('phonics') || text.includes('attention')) {
    return 'cognitive and development';
  }
  
  if (text.includes('calming') || text.includes('aggression') || text.includes('emotional') || text.includes('hyperactivity')) {
